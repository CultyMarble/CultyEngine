...
void ConstantBuffer::Initialize(uint32_t bufferSize)
{
	...
	HRESULT hResult = device->CreateBuffer(&desc, nullptr, &mConstantBuffer);

	ASSERT(SUCCEEDED(hResult), "ConstantBuffer: Failed to create buffer!");
}
...
void CultyEngine::Graphics::ConstantBuffer::Update(const void* data) const
{
