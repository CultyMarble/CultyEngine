...
namespace
{
	std::unique_ptr<CultyEngine::Graphics::GraphicsSystem> sGraphicsSystem;
...
void GraphicsSystem::Initialize(HWND window, bool fullscreen)
{
	...
	ASSERT(SUCCEEDED(hResult), "GraphicsSystem: failed to initialize the device or swap chain!");
...
void GraphicsSystem::Resize(uint32_t width, uint32_t height)
{
	...
	{
		hResult = mSwapChain->ResizeBuffers(0, 0, 0, DXGI_FORMAT_UNKNOWN, 0);
		ASSERT(SUCCEEDED(hResult), "GraphicsSystem: Failed to access swap chain view!");

		mSwapChain->GetDesc(&mSwapChainDesc);
	}

	ID3D11Texture2D* backBuffer = nullptr;
	hResult = mSwapChain->GetBuffer(0, IID_PPV_ARGS(&backBuffer));
	ASSERT(SUCCEEDED(hResult), "GraphicsSystem: Failed to get back buffer!");

	hResult = mD3DDevice->CreateRenderTargetView(backBuffer, nullptr, &mRenderTargetView);
	ASSERT(SUCCEEDED(hResult), "GraphicsSystem: Failed to create render target!");
	...
	hResult = mD3DDevice->CreateTexture2D(&descDepth, nullptr, &mDepthStencilBuffer);
	...
	hResult = mD3DDevice->CreateDepthStencilView(mDepthStencilBuffer, &descDSV, &mDepthStencilView);
	ASSERT(SUCCEEDED(hResult), "GraphicSystem: Failed to create depth stencil view!");
