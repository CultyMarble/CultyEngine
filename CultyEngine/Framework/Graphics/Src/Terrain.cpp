...
void Terrain::Initialize(const std::filesystem::path& fileName, float heightScale)
{
	...
	for (uint32_t z = 0; z < mRows; ++z)
	{
		for (uint32_t x = 0; x < mColumns; ++x)
		{
			...
			vertex.normal = MathC::Vector3::YAxis;
			vertex.tangent = MathC::Vector3::XAxis;
...
float Terrain::GetHeight(const MathC::Vector3& position)
{
...
float CultyEngine::Graphics::Terrain::GetLength() const
{
