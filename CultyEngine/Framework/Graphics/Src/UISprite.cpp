...
void UISprite::SetPosition(const MathC::Vector2& position)
{
    mPosition.x = position.x;
    mPosition.y = position.y;
}

void UISprite::SetScale(const MathC::Vector2& scale)
{
    mScale.x = scale.x;
    mScale.y = scale.y;
}
...
bool UISprite::IsInSprite(float x, float y)
{
    ...
    return x >= mPosition.x - mOrigin.x &&
        x <= mPosition.x + width - mOrigin.x &&
        y >= mPosition.y - mOrigin.y &&
